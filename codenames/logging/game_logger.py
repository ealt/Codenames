...
class GameLogger(BaseLogger):
    ...
    def log_players(self, players: dict[Team, TeamPlayers]) -> None:
        for team, team_players in players.items():
            self.logger.info(
                f'{team} {rp.role_repr(Role.CODEMASTER)}: '
                f'{team_players.codemaster.__class__.__name__}'
            )
            self.logger.info(
                f'{team} {rp.role_repr(Role.INTERPRETER)}: '
                f'{team_players.interpreter.__class__.__name__}'
            )
    ...
    #     self.logger.debug('The interpreter misidentified an agent')

    # def log_exhausted_guesses(self) -> None:
    #     self.logger.debug('The interpreter used all their guesses')

    # def log_pass(self) -> None:
    #     self.logger.debug('The interpreter ended their turn')
